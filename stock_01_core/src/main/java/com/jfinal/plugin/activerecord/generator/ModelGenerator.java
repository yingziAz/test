...
public class ModelGenerator {
	...
	protected void genModelContent(TableMeta tableMeta) {
		...
		genPackage(ret,tableMeta.assignSubPkg);
    ...
    protected void genPackage(StringBuilder ret,String assignSubPkg) {
        String pkgName = modelPackageName;
        if(StrKit.notBlank(assignSubPkg)){
            pkgName +="."+assignSubPkg;
        }
        ret.append(String.format(packageTemplate, pkgName));
    }
    
    protected void genImport(TableMeta tableMeta, StringBuilder ret) {
        String pkgName = baseModelPackageName;
        if(StrKit.notBlank(tableMeta.assignSubPkg)){
            pkgName +="."+tableMeta.assignSubPkg;
        }
        ret.append(String.format(importTemplate, pkgName, tableMeta.baseModelName));
    }
	...
	protected void wirtToFile(TableMeta tableMeta) throws IOException {
        ...
        if(StrKit.notBlank(tableMeta.assignSubPkg)){
            File subDir = new File(modelOutputDir 
                    +(StrKit.notBlank(tableMeta.assignSubPkg)?(File.separator+tableMeta.assignSubPkg):""));
            if (!subDir.exists())
                subDir.mkdirs();
        }
        
		String target = modelOutputDir 
                +(StrKit.notBlank(tableMeta.assignSubPkg)?(File.separator+tableMeta.assignSubPkg):"")
		        + File.separator + tableMeta.modelName + ".java";
