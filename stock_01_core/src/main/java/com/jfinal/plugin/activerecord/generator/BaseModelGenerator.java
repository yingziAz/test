...
public class BaseModelGenerator {
    ...
    protected void genPackage(StringBuilder ret,String assignSubPkg) {
        String packName = baseModelPackageName;
        if(StrKit.notBlank(assignSubPkg)){
            packName +="."+assignSubPkg;
        }
        ret.append(String.format(packageTemplate, packName));
    }
	...
	protected void wirtToFile(TableMeta tableMeta) throws IOException {
        ...
        if(StrKit.notBlank(tableMeta.assignSubPkg)){
            File subDir = new File(baseModelOutputDir 
                    +(StrKit.notBlank(tableMeta.assignSubPkg)?(File.separator+tableMeta.assignSubPkg):""));
            if (!subDir.exists())
                subDir.mkdirs();
        }
		String target = baseModelOutputDir 
		        +(StrKit.notBlank(tableMeta.assignSubPkg)?(File.separator+tableMeta.assignSubPkg):"")
		        + File.separator + tableMeta.baseModelName + ".java";
