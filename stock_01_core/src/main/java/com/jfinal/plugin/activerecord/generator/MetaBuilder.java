...
public class MetaBuilder {
	...
	protected Map<String,String> tablePrefixSubPkgMap = null;
	 ...
	 * 3：开发者若在其他库中发现工作不正常，可通过继承 MetaBuilder并覆盖此方法来实现功能
	 */
	...
	protected void buildTableNames(List<TableMeta> ret) throws SQLException {
		...
		while (rs.next()) {
            ...
            if(tablePrefixSubPkgMap != null){
                for (Map.Entry<String, String> entry : tablePrefixSubPkgMap.entrySet()) {
                    if (tableName.startsWith(entry.getKey())) {
                        tableMeta.tablePrefix = entry.getKey();
                        tableMeta.assignSubPkg = entry.getValue();
                        break;
                    }
                 }
            }
