...
public class PoiKit {
	...
	public HSSFWorkbook export() {
		...
		if (headers.length > 0) {
			...
			for (int h = 0, lenH = headers.length; h < lenH; h++) {
				//comment by su 20160503 begin
				//sheet.addMergedRegion(region);// 到rowTo行columnTo的区域
				//comment by su 20160503 end
				// 得到所有区域
	...
	private static void processAsMap(String[] columns, HSSFRow row,
			Object obj) {
		HSSFCell cell;
	...
	private static void processAsModel(String[] columns, HSSFRow row,
			Object obj) {
		HSSFCell cell;
