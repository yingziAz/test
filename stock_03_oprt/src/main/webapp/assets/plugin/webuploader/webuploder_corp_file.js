jQuery(function() {
	...
	uploader = WebUploader.create({
		...
		fileNumLimit : 1,
		fileSizeLimit : 1 * 50 * 1024 * 1024, // 50 M
	...
	function addFile(file) {
		...
		uploader.makeThumb(file, function (error, src) {
			...
			var minCorpWidth = parseInt(min_width);
			var minCorpHeight = parseInt(min_height);
			if(imgWidth <minCorpWidth || imgHeight<minCorpHeight){
				alertify.alert("图片尺寸过小，请重新上传！");
				uploader.removeFile(file);
				return;
			}
			if(imgWidth >( minCorpWidth * 1.2) || imgHeight> (minCorpHeight*1.2)){
				alertify.alert("图片尺寸过大，请重新上传！");
				uploader.removeFile(file);
				return;
			}
		});
	...
	uploader.on('uploadAccept', function(file, result) {
		if (result.state == "ok") {
			...
			uploadCorpFileComplete(attachField,attachDivId,data.attachId,data.fileName,data.fileSuffix,
					attachNum,position,outWidth,inlineWidth,inlineHeigth,min_width,min_height)

			// 关闭弹窗
			$("#corpFileModal").modal('hide'); // 手动关闭
			$("#corpFileModal").remove();// 清除页面
		}
	});
