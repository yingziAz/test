...
(function ($) {
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        //高级查询
        if(this.options.advSearch){
            html.push(sprintf('<button class="btn' +
                    sprintf(' btn-%s', this.options.buttonsClass) +
                    sprintf(' btn-%s', this.options.iconSize) +
                    '" type="button" name="advSearch" aria-label="advance search" title="%s">',
                    "高级查询"),
                sprintf('<i class="glyphicon glyphicon-filter"></i>', this.options.iconsPrefix),
                '</button>');
        }
        ...
        //高级查询
        if(this.options.advSearch){
            this.$toolbar.find('button[name="advSearch"]')
                .off('click').on('click', $.proxy(this.advSearch, this));
        }
            ...
            if (isIEBrowser()) {
                $search.off('mouseup').on('mouseup', function (event) {
                    ...
                    timeoutId = setTimeout(function () {
                    	...
                    	//add by su 20160507 begin
                    	if(that.options.advSearch && $("#advSearchForm")){
    ...
    //高级查询
    BootstrapTable.prototype.advSearch = function () {
    	var that = this;
    	
    	$('#advSearchModal').modal();
    	
    	//绑定查询事件
    	$("#doAdvSearch").off('click').on('click', function() {
    		that.resetSearch();
    		that.initServer();
    		that.resetView();
    	});
